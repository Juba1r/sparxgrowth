...
import {
  Zap, MapPin, Phone, Mail, ArrowUpRight,
} from "lucide-react";
import { services, industries } from "@/lib/data";

const footerLinks = {
  Services: services.slice(0, 5).map((s) => ({ label: s.shortTitle, href: `/services/${s.slug}` })),
  Industries: industries.slice(0, 5).map((i) => ({ label: i.title, href: `/industries/${i.slug}` })),
  Company: [
...
const contactDetails = [
  { icon: MapPin, value: "123 Liverpool Street, London, EC2M 7PY", color: "#2563eb" },
  ...
  return (
    <footer className="relative bg-[#020818] overflow-hidden flex flex-col items-center">
      ...
      <div
        ...
        style={{ background: "linear-gradient(90deg, transparent 0%, #2563eb 35%, #06b6d4 65%, transparent 100%)" }}
      />

      {/* ── Ambient glows ── */}
      <div className="absolute top-0 left-1/4 w-96 h-96 rounded-full blur-[120px] opacity-[0.07] pointer-events-none"
        style={{ background: "#2563eb" }} />
      <div className="absolute bottom-0 right-1/4 w-96 h-96 rounded-full blur-[120px] opacity-[0.06] pointer-events-none"
          ...
          <div className="flex flex-col lg:flex-row items-start lg:items-center justify-between gap-10">
            ...
            <div className="max-w-sm">
              ...
              <p className="text-white/50 text-sm leading-relaxed mb-6">
                London&apos;s digital growth partner. We combine data-driven strategy with creative execution to help ambitious brands dominate their market.
              </p>
              {/* Social links */}
              ...
              {trust.map((t, i) => (
                ...
                >
                  <span
                    ...
                    style={{ background: "linear-gradient(135deg, #60a5fa, #06b6d4)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent", backgroundClip: "text" }}
                  >
                    {t.stat}
                  </span>
                  <span className="text-xs text-white/40 font-medium">{t.label}</span>
                </motion.div>
              ))}
            </div>
          </div>
                ...
                {contactDetails.map(({ icon: Icon, value, color }) => (
                  <li key={value}>
                    <div className="flex items-start gap-2.5">
                      ...
                      <span className="text-xs text-white/45 leading-relaxed">{value}</span>
                    </div>
                  </li>
                ))}
              </ul>
